...
module.exports = (client, sticker) => {
	...
	if(!logsetting[sticker.guildId]) {
		logsetting[sticker.guildId] = {
			checker: 1,
		};
	}
	if(!log[sticker.guild.id]) return;
	const values = logsetting[sticker.guildId].checker;
	...
	if(values === 1) {
		...
		const logChannel = sticker.guild.channels.cache.get(`${log[sticker.guildId].channel}`);
		if(!logChannel) return;

		const embed = new Discord.MessageEmbed()
			.setAuthor('Sticker Created', sticker.guild.iconURL({ dynamic: true }))
			.setColor('RANDOM')
