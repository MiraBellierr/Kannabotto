...
class Giveaway extends EventEmitter {
	...
	get remainingTimeText() {
		...
		const hourUnit = hours < 2 && (this.messages.units.pluralS || this.messages.units.hours.endsWith('s')) ? this.messages.units.hours.substr(0, this.messages.units.hours.length - 1) : this.messages.units.hours;
		const minuteUnit = minutes < 2 && (this.messages.units.pluralS || this.messages.units.minutes.endsWith('s')) ? this.messages.units.minutes.substr(0, this.messages.units.minutes.length - 1) : this.messages.units.minutes;
		const secondUnit = seconds < 2 && (this.messages.units.pluralS || this.messages.units.seconds.endsWith('s')) ? this.messages.units.seconds.substr(0, this.messages.units.seconds.length - 1) : this.messages.units.seconds;
	...
	async fetchMessage() {
		...
		return new Promise(async (resolve, reject) => {
			...
			const message = await this.channel.messages.fetch(this.messageID).catch((err) => console.error(err));
			if (!message) {
	...
	async checkWinnerEntry(user) {
		...
		const member = guild.members.cache.get(user.id) || (await guild.members.fetch(user.id).catch((err) => console.error(err)));
	...
	async roll(winnerCount = this.winnerCount) {
		...
		let rolledWinners;
		if (!userArray || userArray.length <= winnerCount) {rolledWinners = users.random(Math.min(winnerCount, users.size));}
		else {
			/**
	...
	edit(options = {}) {
		...
		return new Promise(async (resolve, reject) => {
			...
			if (Array.isArray(options.newBonusEntries) && options.newBonusEntries.every((elem) => typeof elem === 'object')) {this.options.bonusEntries = options.newBonusEntries;}
     ...
     */
		...
		return new Promise(async (resolve, reject) => {
			...
			if (winners.length > 0) {
				...
				await this.message.edit({ content: `${this.messages.giveawayEnded}`, embeds: [embed] }).catch((err) => console.error(err));
				...
				if (messageString.length <= 2000) {this.message.reply(messageString);}
				else {
					this.message.reply(
					...
					while (formattedWinners.length >= 2000) {
						await this.message.reply(formattedWinners.substr(0, formattedWinners.lastIndexOf(',', 1999)) + ',');
				...
				this.message.edit({ content: `${this.messages.giveawayEnded}`, embeds: [embed] }).catch((err) => console.error(err));
				resolve([]);
			}
		});
	}
