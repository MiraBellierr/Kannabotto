		...
		if (message.mentions.channels.first()) {
			...
			if (confirm.first().content.toLowerCase() === 'yes') {
				...
				fs.writeFile('./database/leave.json', JSON.stringify(file, null, 2), err => {
					...
					message.channel.send(`Leave message has been set! Type \`${bot_prefix}setleave on\` to turn on the leave message`);
				});
			}
			...
			if (!file[message.guild.id]) return message.channel.send('You haven\'t set up a leave message yet.');
			...
			return message.channel.send(`The right syntax is \`${bot_prefix}setleave <#channel | on | off>\``);
		}
	},
};