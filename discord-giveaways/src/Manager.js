...
class GiveawaysManager extends EventEmitter {
	...
	_checkGiveaway() {
		...
		this.giveaways.forEach(async (giveaway) => {
			...
			giveaway.message.edit({ content: `${giveaway.messages.giveaway}`, embeds: [embed] }).catch((err) => console.error(err));
			...
			if (giveaway.lastChance.enabled && (giveaway.remainingTime - giveaway.lastChance.threshold) < this.options.updateCountdownEvery) {
				setTimeout(() => {
					...
					giveaway.message.edit({ content: `${giveaway.messages.giveaway}`, embeds: [embed2] }).catch((err) => console.error(err));
	...
	async _handleRawPacket(packet) {
		...
		const member =
            ...
            (await guild.members.fetch(packet.d.user_id).catch((err) => console.error(err)));
		if (!member) return;
