...
module.exports = {
	...
	run: async (client, message, args) => {
		...
		if (args[0] === 'default') {
			...
			const h = await message.reply('*Please Wait...*');

			fs.writeFile('./database/background.json', JSON.stringify(bg, null, 2), (err) => {
				if (err) {
					message.reply(`Something went wrong.. \`${err}\`. Please report it to the support server.`);
				}
				else {
					message.reply('Your background image has been set!').then(()=>{ h.delete();});
				}
			});
			return;
		}
